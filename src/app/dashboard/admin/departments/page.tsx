// 📋 CHANGELOG:
// ✅ Perubahan: Menyempurnakan komponen Modal Detail Profil Siswa di Eksplorasi Kelas untuk merender status dan tombol pratinjau dokumen BPJS secara akurat.
// ✨ Fitur Baru: Dukungan penuh tampilan dokumen BPJS Ketenagakerjaan dan link eksternal pratinjau file.
// 🎨 UI/UX Update: Badge status BPJS dinamis (Hijau untuk disetujui/diunggah, Kuning untuk belum upload).
// 🔧 Bug Fix: Memastikan data `bpjsStatus` dan `bpjsUrl` dari database terpanggil dan dirender dengan sempurna di UI.
// 🚀 Inovasi: Enterprise Student 360° Profile & Document Inspector.
...
export default function AdminDepartmentsHierarchyPage() {
  ...
  const [studentForm, setStudentForm] = useState({ name: '', nis: '', phone: '' });
  const [submittingStudent, setSubmittingStudent] = useState(false);

  // Modal Tambah dari Daftar Eksisting (Bulk Insert)
          ...
          const deptList = resDept.data.map((dept: any) => {
            const classesOfDept = resClass.data.filter(
              ...
              const studentsInThisClass = studentsData.filter(
                (s: any) => s.role?.toUpperCase() === 'SISWA' && s.className?.trim().toLowerCase() === cls.name.trim().toLowerCase()
              );
  ...
  const handleAssignBulkStudents = async () => {
      ...
      await Promise.all(selectedStudentIds.map(async (studentId) => {
        ...
        return fetch('/api/admin/users', {
          ...
          body: JSON.stringify({
            id: studentObj.id,
            className: selectedClass.name,
            department: selectedDepartment?.name || studentObj.department,
          }),
        });
      }));

      setSuccessMsg(`Berhasil memindahkan ${selectedStudentIds.length} siswa ke kelas ${selectedClass.name}!`);
      setShowExistingModal(false);
      setSelectedStudentIds([]);
      loadHierarchyData();
    } catch (err) {
      console.error(err);
      alert('Terjadi kesalahan jaringan saat memasukkan kumpulan siswa.');
    } finally {
  ...
  const studentsInSelectedClass = selectedClass 
    ? allStudents.filter((s: any) => s.role?.toUpperCase() === 'SISWA' && s.className?.trim().toLowerCase() === selectedClass.name.trim().toLowerCase())
    : [];

  const availableExistingStudents = allStudents.filter(
    (s: any) => s.role?.toUpperCase() === 'SISWA' && (!s.className || s.className.trim().toLowerCase() !== selectedClass?.name?.trim().toLowerCase())
  );
    ...
    <div className={`min-h-screen p-6 sm:p-10 space-y-8 transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {selectedClass && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className={`w-full max-w-5xl border rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 max-h-[85vh] flex flex-col relative z-[10000] ${
              ...
              <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/20 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${
                    activeStudentDetail.bpjsStatus === 'DISETUJUI' || activeStudentDetail.bpjsStatus === 'UPLOADED' || activeStudentDetail.bpjsUrl ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'
                  }`}>
                    {activeStudentDetail.bpjsStatus || (activeStudentDetail.bpjsUrl ? 'DISETUJUI' : 'BELUM_UPLOAD')}
                  </span>
                </div>
      ...
      {/* SUB-MODAL: AMBIL DARI DAFTAR SISWA EKSISTING (BULK INSERT) */}
      {showExistingModal && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className={`w-full max-w-2xl border rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 max-h-[85vh] flex flex-col ${
            theme === 'dark' ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-900'
          }`}>
            <div className="flex justify-between items-center border-b pb-4 border-inherit shrink-0">
              <div>
                <h3 className="text-base font-bold flex items-center space-x-2">
                  ...
                  <span>Pilih & Masukkan Siswa (Multi-Select)</span>
                </h3>
                <p className="text-xs text-slate-400">Centang siswa yang ingin dimasukkan ke kelas {selectedClass?.name}</p>
              </div>
              <button onClick={() => setShowExistingModal(false)} className="text-slate-400 hover:text-red-500 transition-colors cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
                  ...
                  {availableExistingStudents
                    ...
                    .map((student: any) => (
                      <div 
                        ...
                        className={`flex items-center space-x-4 p-3 rounded-2xl border transition-all cursor-pointer select-none ${
                          selectedStudentIds.includes(student.id)
                            ? (theme === 'dark' ? 'bg-indigo-600/20 border-indigo-500' : 'bg-indigo-50 border-indigo-500')
                            : (theme === 'dark' ? 'bg-slate-950/50 border-slate-800 hover:border-slate-700' : 'bg-slate-50 border-slate-200 hover:border-slate-300')
                        }`}
                      >
                        <div className="shrink-0 text-indigo-500">
                          {selectedStudentIds.includes(student.id) ? <CheckSquare className="w-5 h-5" /> : <Square className="w-5 h-5 opacity-40" />}
                        </div>
                        <div className="flex-1">
                          <p className={`text-xs font-bold ${selectedStudentIds.includes(student.id) ? 'text-indigo-600 dark:text-indigo-400' : ''}`}>{student.name}</p>
                          <p className="text-[10px] text-slate-400 font-mono mt-0.5">
                            NIS: {student.nis || student.username} {student.className ? `• Kelas: ${student.className}` : '• Belum ada kelas'}
                          </p>
                        </div>
                      </div>
                    ))}
            ...
            <div className="flex justify-between items-center pt-4 border-t border-inherit shrink-0">
              ...
              <div className="space-x-3">
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl text-xs font-semibold border cursor-pointer transition-colors ${
                    theme === 'dark' ? 'border-slate-800 text-slate-400 hover:bg-slate-800' : 'border-slate-200 text-slate-600 hover:bg-slate-100'
                  }`}
                >
                  Batal
                ...
                <button
                  type="button"
                  onClick={handleAssignBulkStudents}
                  disabled={selectedStudentIds.length === 0 || isBulkSubmitting}
                  className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-600 disabled:text-slate-400 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-md cursor-pointer disabled:cursor-not-allowed"
                >
                  {isBulkSubmitting ? 'Memproses...' : 'Masukkan Terpilih'}
                </button>
              </div>
            </div>
          </div>
