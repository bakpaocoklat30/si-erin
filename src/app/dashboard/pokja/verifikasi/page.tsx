      ...
      <div className={`p-6 rounded-3xl border shadow-lg flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 ${
        ...
        <button
          ...
          className={`px-4 py-3 rounded-2xl border text-xs font-bold flex items-center space-x-2.5 transition-all cursor-pointer ${
            isMasterAllSelected
              ...
              : theme === 'dark' ? 'bg-slate-800/60 border-slate-700 text-slate-300 hover:bg-slate-800' : 'bg-slate-100 border-slate-300 text-slate-800 hover:bg-slate-200'
          }`}
        >
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className={`w-full pl-11 pr-4 py-3 rounded-2xl text-xs font-semibold border outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500 ${
              theme === 'dark' ? 'bg-slate-950 border-slate-800 text-slate-100 focus:border-indigo-500' : 'bg-slate-50 border-slate-300 text-slate-900 focus:border-indigo-500 shadow-sm'
            }`}
          />
        </div>
      </div>
      ...
      <div className="space-y-6">
        {filteredGroupedData.length > 0 ? (
          filteredGroupedData.map((group) => {
            ...
            const groupIds = groupPlacements.map((p: any) => p.id);
            const isGroupAllSelected = groupIds.length > 0 && groupIds.every((id: string) => selectedPlacementIds.includes(id));
            const selectedInGroupCount = groupIds.filter((id: string) => selectedPlacementIds.includes(id)).length;
                ...
                <div className={`p-6 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-colors ${
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <h3 className="font-extrabold text-base text-slate-900 dark:text-indigo-400">{group.industryName}</h3>
                        <span className="px-3 py-0.5 rounded-full text-[11px] font-extrabold bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border border-indigo-500/20 flex items-center space-x-1">
                          <Calendar className="w-3 h-3" />
                          <span>{group.periodName}</span>
                        </span>
                      </div>
                      <p className="text-xs text-slate-700 dark:text-slate-400 font-medium line-clamp-1">{group.industryAddress}</p>

                      {/* 🌟 WIDGET KAPASITAS KUOTA DUDI */}
                      <div className="flex flex-wrap items-center gap-2.5 pt-1">
                        {/* BADGE STATUS KUOTA */}
                        {group.isUnlimited ? (
                          <span className="px-2.5 py-1 rounded-xl text-[10px] font-black bg-blue-500/10 text-blue-700 dark:text-blue-400 border border-blue-500/30 flex items-center space-x-1.5 shadow-sm">
                            <span className="w-1.5 h-1.5 rounded-full bg-blue-600 dark:bg-blue-400 animate-pulse"></span>
                            <span>KUOTA BEBAS</span>
                            <span className="text-[10px] text-blue-800 dark:text-blue-300 font-bold">• Terisi {group.verifiedCount || 0} Siswa</span>
                          </span>
                        ) : group.isFull ? (
                          <span className="px-2.5 py-1 rounded-xl text-[10px] font-black bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-500/30 flex items-center space-x-1.5 shadow-sm">
                            <AlertCircle className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400 shrink-0" />
                            <span>KUOTA PENUH</span>
                            <span className="text-[10px] text-rose-800 dark:text-rose-300 font-bold">• {group.verifiedCount}/{group.totalQuota} Siswa</span>
                          </span>
                        ) : (group.remainingNumber ?? 0) < groupPlacements.length ? (
                          <span className="px-2.5 py-1 rounded-xl text-[10px] font-black bg-amber-500/10 text-amber-800 dark:text-amber-400 border border-amber-500/30 flex items-center space-x-1.5 shadow-sm">
                            <AlertTriangle className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 shrink-0" />
                            <span>SISA KUOTA MENIPIS: {group.remainingQuota} Kursi</span>
                            <span className="text-[10px] text-amber-900 dark:text-amber-300 font-bold">• Terisi {group.verifiedCount}/{group.totalQuota}</span>
                          </span>
                        ) : (
                          <span className="px-2.5 py-1 rounded-xl text-[10px] font-black bg-emerald-500/10 text-emerald-800 dark:text-emerald-400 border border-emerald-500/30 flex items-center space-x-1.5 shadow-sm">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
                            <span>SISA KUOTA: {group.remainingQuota} Kursi</span>
                            <span className="text-[10px] text-emerald-900 dark:text-emerald-300 font-bold">• Terisi {group.verifiedCount}/{group.totalQuota}</span>
                          </span>
                        )}

                        {/* MINI PROGRESS BAR KETERISIAN KUOTA */}
                        {!group.isUnlimited && (group.quotaNumber ?? 0) > 0 && (
                          <div className="hidden sm:flex items-center space-x-2 bg-slate-100 dark:bg-slate-950/80 px-2.5 py-1 rounded-xl border border-slate-300 dark:border-slate-800">
                        ...
                        {group.verifiedCount > 0 && (
                          <button
                            ...
                            className="px-2.5 py-1 rounded-xl text-[10px] font-extrabold bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30 flex items-center space-x-1 transition-all cursor-pointer shadow-sm"
                  ...
                  <div className="flex flex-col items-end gap-2 self-end md:self-auto text-xs">
                    ...
                    {!group.isUnlimited && selectedInGroupCount > (group.remainingNumber ?? 0) && (
                      <div className="px-2.5 py-1 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-700 dark:text-rose-400 text-[10px] font-black flex items-center space-x-1.5 animate-pulse">
                    ...
                    <div className="flex items-center space-x-4">
                      <span className="text-slate-700 dark:text-slate-400 font-semibold">
                        Terpilih: <strong className="text-indigo-700 dark:text-indigo-400">{selectedInGroupCount}</strong>/{groupPlacements.length} Siswa
                      </span>
                      ...
                      <button
                        ...
                        className={`p-2 rounded-xl transition-all cursor-pointer ${
                          theme === 'dark' ? 'bg-slate-800 hover:bg-slate-700 text-slate-300' : 'bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300'
                        }`}
                      >
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs">
                      <thead className={`border-b text-[10px] uppercase font-bold tracking-wider ${
                        theme === 'dark' ? 'bg-slate-950/40 border-slate-800 text-slate-400' : 'bg-slate-100 border-slate-200 text-slate-700'
                      }`}>
                        <tr>
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="p-4 font-semibold">
                                <div className="font-bold text-sm text-slate-900 dark:text-indigo-300">{p.student?.name}</div>
                                <div className="text-[11px] text-slate-700 dark:text-slate-400 font-mono">NIS: {p.student?.nis}</div>
                              </td>

                              <td className="p-4">
                                <div className="font-bold text-slate-900 dark:text-slate-200">{p.student?.className}</div>
                                <div className="text-[11px] text-slate-700 dark:text-slate-400 font-medium">{p.student?.department}</div>
                              </td>
          ...
          <div className={`p-12 text-center text-xs font-semibold rounded-3xl border ${
            theme === 'dark' ? 'text-slate-400 border-slate-800 bg-slate-900/40' : 'text-slate-700 border-slate-200 bg-white'
          }`}>
            ...
            <div className="p-4 border-t border-inherit flex justify-end space-x-3">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  theme === 'dark' ? 'bg-slate-800 hover:bg-slate-700 text-slate-300' : 'bg-slate-200 hover:bg-slate-300 text-slate-800'
                }`}
              >
                Tutup Pratinjau
              </button>
          ...
          <div className={`w-full max-w-2xl max-h-[85vh] rounded-3xl border shadow-2xl flex flex-col overflow-hidden ${
            ...
            <div className="p-6 border-b border-inherit flex justify-between items-center bg-indigo-500/10">
              <div className="space-y-1">
                <div className="flex items-center space-x-2 text-indigo-700 dark:text-indigo-400 text-xs font-bold uppercase tracking-wider">
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs">
              ...
              <div className="grid grid-cols-3 gap-3 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 text-center">
                <div className="space-y-0.5">
                  <span className="text-[10px] text-slate-700 dark:text-slate-400 font-bold uppercase">Total Kuota</span>
                ...
                <div className="space-y-0.5 border-x border-inherit">
                  <span className="text-[10px] text-slate-700 dark:text-slate-400 font-bold uppercase">Sudah Terisi</span>
                  <p className="text-base font-black text-emerald-700 dark:text-emerald-400">
                    {quotaModalGroup.verifiedCount || 0} Siswa
                  </p>
                </div>
                <div className="space-y-0.5">
                  <span className="text-[10px] text-slate-700 dark:text-slate-400 font-bold uppercase">Sisa Kuota</span>
                  <p className={`text-base font-black ${quotaModalGroup.isFull ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-700 dark:text-emerald-400'}`}>
              ...
              {/* List Siswa */}
              <div className="space-y-2">
                <h4 className="font-extrabold text-xs text-slate-800 dark:text-slate-400 uppercase tracking-wider flex items-center space-x-1.5">
                ...
                {quotaModalGroup.verifiedStudents && quotaModalGroup.verifiedStudents.length > 0 ? (
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {quotaModalGroup.verifiedStudents.map((st: any, idx: number) => (
                      ...
                      >
                        ...
                        <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-emerald-500/10 text-emerald-800 dark:text-emerald-400 border border-emerald-500/20 whitespace-nowrap">
                  ...
                  <div className="text-center py-6 text-slate-600 dark:text-slate-400 border border-dashed rounded-xl border-slate-300 dark:border-slate-700">
