...
export default function PersuratanSppdPage() {
  ...
  // URL Objek Blob untuk live preview PDF yang diunggah & Halaman aktif yang sedang diperiksa
            ...
            {!bulkUploadModal.commitResult ? (
              ...
              >
                ...
                >
                  ...
                  {/* 3. Status File, Quick Page Ribbon & Aksi Analisis */}
                  <div className="space-y-2 pt-1">
                    ...
                    {/* 🌟 Quick Page Selector Ribbon: Klik halaman untuk langsung melihat di panel pratinjau */}
                    {Boolean(bulkUploadModal.analysisResult?.totalPages) && (
                      <div className="p-3 bg-slate-100/90 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700/60 space-y-1.5">
                        <div className="flex items-center justify-between text-[11px] font-bold text-slate-500">
                          <span>Pilih & Periksa Halaman PDF:</span>
                          <span className="text-[10px] font-medium text-slate-400">Klik angka halaman untuk melompat ke halaman tersebut</span>
                        </div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {Array.from({ length: bulkUploadModal.analysisResult!.totalPages }, (_, i) => i + 1).map((pageNum) => {
                            const isCurrent = activePreviewPage === pageNum;
                            const mappedItem = bulkUploadModal.mappings.find((m) =>
                              parsePageNumbers(m.pageNumbersStr).includes(pageNum)
                            );
                            return (
                              <button
                                key={pageNum}
                                ...
                                className={`px-2.5 py-1 rounded-xl text-xs font-black transition-all cursor-pointer flex items-center gap-1 shadow-sm ${
                                  isCurrent
                                    ? 'bg-emerald-600 text-white ring-2 ring-emerald-500/50 scale-105'
                                    : mappedItem
                                    ? mappedItem.docType === 'TUGAS'
                                ...
                                title={`Halaman ${pageNum}${mappedItem ? ` (${mappedItem.docType === 'TUGAS' ? 'Surat Tugas' : 'SPPD'})` : ' (Belum dipetakan)'}`}
                              >
                                <span>Hal {pageNum}</span>
                                {mappedItem && (
                                  <span className="text-[9px] opacity-75 font-normal">
                                    {mappedItem.docType === 'TUGAS' ? '• Tugas' : '• SPPD'}
                                  </span>
                                )}
                              </button>
                            );
                    ...
                    <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 space-y-4 animate-in fade-in">
                      ...
                      <div className="space-y-3 max-h-[360px] overflow-y-auto pr-1">
                        {bulkUploadModal.mappings.map((item, idx) => {
                          const parsedPages = parsePageNumbers(item.pageNumbersStr);
                          const isRowActive = parsedPages.includes(activePreviewPage);

                          return (
                            <div
                              key={item.id}
                              onClick={() => {
                                if (parsedPages.length > 0 && !isRowActive) {
                                  setActivePreviewPage(parsedPages[0]);
                                }
                              }}
                              className={`p-3.5 bg-white dark:bg-slate-900 rounded-2xl border shadow-sm space-y-2.5 transition-all cursor-pointer ${
                                isRowActive
                                  ? 'border-emerald-500 ring-2 ring-emerald-500/25 bg-emerald-50/15 dark:bg-emerald-950/20'
                              ...
                              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                  <span className={`w-6 h-6 rounded-full font-black text-xs flex items-center justify-center shrink-0 ${
                                    isRowActive ? 'bg-emerald-600 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
                                  }`}>
                                    {idx + 1}
                                  </span>
                                  <select
                                ...
                                <div className="flex items-center gap-1.5 shrink-0">
                                  ...
                                  <input
                                    ...
                                    onFocus={() => {
                                      if (parsedPages.length > 0) setActivePreviewPage(parsedPages[0]);
                                    }}
                                    onChange={(e) => {
                                      handleUpdateMappingRow(item.id, 'pageNumbersStr', e.target.value);
                                      const pList = parsePageNumbers(e.target.value);
                                      if (pList.length > 0) setActivePreviewPage(pList[0]);
                                    }}
                                  ...
                                  <div className="flex items-center gap-1 shrink-0">
                                    {parsedPages.map((p) => (
                                      <button
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          setActivePreviewPage(p);
                                        }}
                                        className={`px-2 py-1 rounded-lg text-[10px] font-black transition-all cursor-pointer shadow-xs ${
                                          activePreviewPage === p
                                            ? 'bg-emerald-600 text-white shadow-sm ring-1 ring-emerald-500'
                                            : 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'
                                        }`}
                                        title={`Klik untuk melihat Halaman ${p} di panel pratinjau`}
                                      >
                                        👁️ Hal {p}
                                      </button>
                                    ))}
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleRemoveMappingRow(item.id);
                                  }}
                ...
                {bulkPdfPreviewUrl && bulkUploadModal.showPdfPreview && (
                  <div className="lg:col-span-5 h-[620px] bg-slate-100 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col sticky top-0 shadow-inner">
                    {/* Header Bar Viewer */}
                    <div className="px-3.5 py-2.5 bg-slate-200/80 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between gap-2 shrink-0">
                      <div className="flex items-center gap-1.5 shrink-0">
                        <FileText className="w-4 h-4 text-emerald-500 shrink-0" />
                        <span className="text-xs font-black text-slate-800 dark:text-slate-200 hidden sm:inline">
                          Pratinjau PDF
                        </span>
                      </div>

                      {/* Navigasi Halaman PDF [ ◀ Hal X / Y ▶ ] */}
                      <div className="flex items-center gap-1.5 bg-white dark:bg-slate-900 px-2 py-1 rounded-xl border border-slate-300 dark:border-slate-700 shadow-xs">
                        <button
                          type="button"
                          onClick={() => setActivePreviewPage((prev) => Math.max(1, prev - 1))}
                          disabled={activePreviewPage <= 1}
                          className="p-1 text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 disabled:opacity-30 disabled:hover:text-slate-400 cursor-pointer transition-colors"
                        ...
                        <div className="flex items-center gap-1 text-xs font-bold text-slate-700 dark:text-slate-200">
                          <span className="text-[11px] text-slate-400 font-semibold">Hal</span>
                          <span className="px-1.5 py-0.5 bg-emerald-50 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 rounded font-black min-w-[20px] text-center">
                            {activePreviewPage}
                          </span>
                          <span className="text-slate-400">/</span>
                          <span className="text-slate-500 dark:text-slate-400">
                            {bulkUploadModal.analysisResult?.totalPages || '?'}
                          </span>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const maxP = bulkUploadModal.analysisResult?.totalPages || 999;
                            setActivePreviewPage((prev) => Math.min(maxP, prev + 1));
                          }}
                          disabled={
                            Boolean(
                              bulkUploadModal.analysisResult?.totalPages &&
                              activePreviewPage >= bulkUploadModal.analysisResult.totalPages
                            )
                          }
                          className="p-1 text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 disabled:opacity-30 disabled:hover:text-slate-400 cursor-pointer transition-colors"
                      ...
                      {/* Kontrol Kanan: Tab Baru & Tutup */}
                      <div className="flex items-center gap-2 shrink-0">
                        <a
                          href={`${bulkPdfPreviewUrl}#page=${activePreviewPage}`}
                          target="_blank"
                          rel="noreferrer"
                          className="text-[11px] font-semibold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"
                          title={`Buka Halaman ${activePreviewPage} di Tab Baru`}
                        >
                    ...
                    {/* Sub-bar Informasi Pemetaan Halaman Aktif */}
                    {(() => {
                      const activeMapping = bulkUploadModal.mappings.find((m) =>
                        parsePageNumbers(m.pageNumbersStr).includes(activePreviewPage)
                      );
                      const activeTask = activeMapping
                        ? (bulkUploadModal.analysisResult?.assignments || tasks).find(
                            (a: any) => a.id === activeMapping.assignmentId
                          )
                        : null;

                      return (
                        <div className="px-3 py-1.5 bg-slate-50 dark:bg-slate-900/70 border-b border-slate-200/80 dark:border-slate-800 flex items-center justify-between text-[11px] text-slate-600 dark:text-slate-300 shrink-0">
                          <div className="flex items-center gap-1.5 truncate">
                            {activeMapping ? (
                              <>
                                <span
                                  className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                                    activeMapping.docType === 'TUGAS'
                                      ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300'
                                      : 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300'
                                  }`}
                                >
                                  {activeMapping.docType === 'TUGAS' ? 'Surat Tugas' : 'SPPD'}
                                </span>
                                <span className="font-semibold truncate text-slate-800 dark:text-slate-200">
                                  {activeTask?.teacher?.name || activeTask?.teacherName || 'Guru belum dipilih'}
                                </span>
                              </>
                            ) : (
                              <span className="text-slate-400 italic">
                                Halaman ini belum dipetakan ke dokumen manapun
                              </span>
                            )}
                          </div>
                          <span className="text-[10px] text-slate-400 shrink-0 ml-2">
                            Hal {activePreviewPage}
                          </span>
                        </div>
                      );
                    })()}
                    ...
                    {/* PDF Viewer Frame */}
                    <div className="relative flex-1 w-full bg-slate-200 dark:bg-slate-950 overflow-hidden flex flex-col">
                      <iframe
                        key={`pdf_frame_${activePreviewPage}_${bulkPdfPreviewUrl}`}
                        src={`${bulkPdfPreviewUrl}#page=${activePreviewPage}&zoom=page-fit&toolbar=1&navpanes=0`}
                        title={`Pratinjau PDF Halaman ${activePreviewPage}`}
