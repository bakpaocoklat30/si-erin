...
export default withAuth(
  function middleware(req: any) {
    ...
    response.headers.set(
      ...
      "default-src 'self'; script-src 'self' 'unsafe-eval' 'unsafe-inline' unpkg.com cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' unpkg.com fonts.googleapis.com; img-src 'self' data: blob: https:; font-src 'self' fonts.gstatic.com; frame-src 'self' blob: data:; object-src 'self' blob: data:; frame-ancestors 'self';"
    );
